...
```scala
/**
*This file decides how many votes are required to implement changes to the network for softforks and everyday changes respectively. We will call upon our new parameter "MinimumVoteAccuracy" that define a minimum acurracy that votes may be and divide our votesCount and count by the value.
*/
...
case class VotingSettings(votingLength: Int,
                          ...
                          minimumVoteAccuracy: Int,
                          version2ActivationHeight: Int,
                          version2ActivationDifficultyHex: String) {

  def softForkApproved(votesCount: Int): Boolean = votesCount / minimumVoteAccuracy > votingLength * softForkEpochs * 9 / 10

  def changeApproved(count: Int): Boolean = count / minimumVoteAccuracy> votingLength / 2
}
```
...
/**
As in the above file, we are no longer increasing votes by '1' but rather by a fraction of our new parameter VoteAccuracy.
*/
