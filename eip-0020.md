...
The `ErgoPaySigningRequest` must not contain `reducedTx` and `reducedTxList` fields.

Signed transactions are submitted to Ergo network in their order in the `reducedTxList` array.
