...
- no need to add change outputs explicitly
- transactions can be sent in any order and don't depend on each other
- any transaction can be cancelled/delayed/queued without affecting others

#### Conclusion

The introduction of Pooled Transaction Inputs significantly streamlines the process of
managing multiple DeFi interactions for users like Alice. By allowing transactions to
specify input pools rather than fixed UTXOs, Alice can engage in various DeFi protocols
simultaneously without the need for chaining transactions or managing UTXO dependencies.
This approach not only enhances the flexibility and efficiency of transaction creation, but
also reduces the risk of transaction conflicts caused by delays/queueing/cancellations of
transactions, enabling a more responsive and fluid DeFi experience on UTXO blockchain
platform such as Ergo.

Furthermore, Pooled Transaction Inputs mitigate the challenges associated with UTXO
consolidation, allowing users to maximize their participation in the DeFi ecosystem
without being bottlenecked by transaction processing limitations. This proposal
represents a step forward in the scalability and usability of the Ergo
blockchain, paving the way for more complex and dynamic DeFi applications.

Pooled Transaction Inputs simplifies UTXO management in the wallet software, by allowing
transactions to draw from UTXO pools. This eliminates the need
for fixed UTXO selection, enhancing transaction flexibility and efficiency. As a result,
users enjoy a smoother DeFi experience on the Ergo blockchain.
