...
* We create new consensus-level checks that verify that the proper amount of re-emission tokens are going from the emission box to a miner rewards box, and also that during spending a box which is not an emission box but contains re-emission tokens (miner rewards box), the tokens are being burnt, and the same amount of nanoERG is locked by the re-emission contract. 
...
```scala
    ...
    // TODO: describe why this condition can replace 5 others (heightCorrect, etc)
    val sponsored = {
