...
* otherwise, if block reward R is less than 15 ERG, send R - 3 ERG from it to the reemission contract
...
Emission in Ergo is done via a contract which is locking funds existing since before genesis block. Changing emission then in
...
```scala
    // todo: check reemission contract NFT id
