...
- [Proposed Solution](#proposed-solution)
    ...
    - [Implementation Details](#implementation-details)
        ...
        - [Deterministic Box Selection](#deterministic-box-selection)
            - [UTXO Storage and Key Design](#utxo-storage-and-key-design)
...
(using lexically ordered keys). This will allow to implement simple FIFO selection of
UTXOs [1] on-chain, as part of Layer 1 protocol.
...
- `addressOpt`: This is the optional pool's address associated with the UTXO.
- 
...
These fields are calculated in the `UtxoState.stateChanges` method during the processing
of the full block. 
...
stateChanges.foreach {
  case (Insert(boxId, boxBytes), utxo) =>
    utxo.addressOpt.foreach { address =>
...
the above information (serialized UTXO bytes followed by serialized `blockHeight` and
`outputIndex`).
...
means PooledInput mechanism is fully transparent to the smart contracts evaluation.
...
**Research Papers and Proposals**
 ...
 Various academic papers discuss coin selection
 strategies, focusing on optimizing transaction costs and sizes. These discussions often
 highlight the limitations of off-chain coin selection and suggest improvements, some of
 which could potentially be integrated into the protocol layer. For example, the paper "A
 Survey on Coin Selection Algorithms in UTXO-based Blockchains" provides a comprehensive
 review and categorizes existing algorithms, potentially paving the way for on-chain
 implementations [1].
