...
- [Ergo Transaction Validation With JIT Costing](#ergo-transaction-validation-with-jit-costing) 
  ...
  - [Default Transaction Cost](#default-transaction-cost)
...
The `MaxTxCost` and `DefaultTxCost` parameters are described in the [section below](#default-transaction-cost).
...
`costLimit` will be selected by the verifier (see [details](#default-transaction-cost))
...
the transactions with missing `tx.feeOut.R4` value will be processed with [default
transaction cost limit](#default-transaction-cost).
