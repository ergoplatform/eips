...
1. `emission(NFT) == 1`
2. `emission(LP) == K`, where `K` is predefined total `LP` supply
...
DexFeePerToken | Long       | DEX fee in nanoERGs per one unit of quote asset
PoolScriptHash | Coll[Byte] | A hash of a pool contract
