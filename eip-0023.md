...
7. Transferable oracle tokens. Oracle tokens are free to be transferred between public keys
...
|Oracle | 15 | Oracle token <br /> Reward tokens | R4: Public key (GroupElement) <br> R5: Pool box Id (Coll[Byte])<br> R6: Published rate (Long) | Publish data-point <br /> Accumulate reward tokens | Publish data-point, <br /> Refresh pool, <br /> Transfer oracle token, <br /> Extract reward tokens|
...
2. The second input is a refresh box that contains the following logic ([smart contract](#refresh-contract)):
    ...
    - Any value published within the last epoch length by someone holding the oracle token is considered *latest*. This is called an oracle box. 
      ...
      - Register R5 must be the box id of the stale pool box.
...
3. The following rules (not enforced by the smart contract) to be followed.
    ...
    - It should store box id of the current pool box in R5.
...
This is similar to the "Publish data-point" case, except that Step 3 is modified as follows. 
...
3. The following rules (not enforced by the smart contract) to be followed.
    - It should store new owner's group element in R4.
    - It should keep at least 1 reward token.
...
{ // This box (refresh box)
  ...
  val oracleTokenId = fromBase64("ERERERERERERERERERERERERERERERERERERERERERE=") // TODO replace with actual
