...
* support for conversion from nbits-encoded number to big integer and back, to provide support to check difficulty for 
 Ergo (and Bitcoin) headers efficiently. Examples for 

Activation 
----------

To have old clients supporting 6.0 changes after soft-fork activation, some script deserialization validation rules 
should be replaced with identical ones but having different ids. 

Rule 1011 should be replaced with another one, likely with the same rule but under different id.

Rule 1007 should be replaced with another one, likely with the same rule but under different id.

Rule 1008 should be replaced with another one, likely with the same rule but under different id.
...
* implement BigIntMod type for cryptographic applications
...
status: costing missing, unpolished, get_eval issue
