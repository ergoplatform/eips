...
- [Proposed Solution](#proposed-solution)
    ...
    - [Double-Spending Problem with PooledInputs](#double-spending-problem-with-pooledinputs)
    - [Solution to Double-Spending Problem](#solution-to-double-spending-problem)
...
6. **Change Output Handling:**
  - After selecting the required UTXOs, the change output will be added with
    ...
    thus can be consumed by the next transaction spending from the same pool (respecting FIFO
    ordering).
...
### Double-Spending Problem with PooledInputs
...
All transaction are independent and can be submitted simultaneously without the risk of 
cancellation/delaying/queueing of the parent transaction affecting them all.
...
### PooledInput Data Structure
   ...
   case class PooledInput(
     ...
     signature: Array[Byte] // Signature for the `address` sigma proposition proof
   )
   ```
  - `SigmaProp` type: aka Sigma Proposition, specifies the source address of the UTXOs (in
    its simplest form it is P2PK, but can also be multisig and more complex [sigma trees
    supported by Ergo](https://hackernoon.com/sigma-protocols-for-the-working-programmer)).
 ...
 The `address` field defines the UTXO _pool_ as a list of boxes as if returned by the following query:
 ```sql
...
### Transaction Format Extension
   ...
   case class ErgoTransaction(
     ...
     dataInputs: Seq[DataInput],  // List of data inputs (readonly or reference inputs)
...
corresponding `PooledInput`s.
  - Technically, the format versioning can be done by reusing one of the counts (e.g. data
    input count). When the count is greater than MaxArrayLength protocol constant 
    (e.g. 100000) then it means the transaction contains `pools` field, otherwise the list is empty,
    which means the transaction is indistinguishable from the current protocol version. This
    is similar to how versioning is done in BlockTransactionsSerializer.
...
stateChanges.foreach {
  ...
  case (Remove(boxId), utxo)  =>
    utxo.addressOpt match { 
      case Some(address) =>
          // When UTXO is spent from a pool, remove it from the indexDb  
          val key = createKey(address, utxo.blockHeight, utxo.outputIndex)
          indexDb.delete(key)
      case _ =>
    }
